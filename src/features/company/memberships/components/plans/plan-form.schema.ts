...
const planFormBaseSchema = z.object({
  ...
  duration: z.number({ required_error: 'Duration is required' }).positive('Duration should be a positive number'),
  ...
  price: z.number({ required_error: 'Price is required' }).positive('Price should be a positive number'),
...
export const planFormSchema = planFormBaseSchema
  .refine((data) => data.type !== 'sessions' || (data.sessionsCount != null && data.sessionsCount > 0), {
    message: 'Sessions count is required for session-based plans',
    path: ['sessionsCount'],
  })
